...
import { Paperclip, CornerDownLeft } from 'lucide-react'; // Import necessary icons
...
export default function AI() {
	...
	const generateAnswer = async (e) => {
        ...
        let promptPrefix = 'Imagine you are a professor majoring in Information Technology. Give precise answer to help user solve the problem on their own. Do not answer by code.';
        ...
        try {
            ...
            const formattedAnswerText = answerText.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
		...
		<div className="relative flex h-full min-h-[85vh] ml-16 flex-col rounded-xl border mt-2 p-4 flex-1">
			...
			<div className="flex-1 overflow-auto p-4 mt-4 mb-4 max-h-[32rem]">
				{chatHistory.map((chat, index) => (
					<div key={index} className={`mb-4 rounded-lg p-2 ${chat.type === 'question' ? 'bg-blue-100 text-right' : 'bg-gray-100'}`}>
						{chat.type === 'question' ? (
							<ReactMarkdown>{chat.text}</ReactMarkdown>
						) : (
							<div dangerouslySetInnerHTML={{ __html: chat.text }} className="whitespace-pre-line" />
						)}
					</div>
				))}
			</div>
